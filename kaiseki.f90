program kaiseki
   ...
   !初期値
   real(8) :: X_in
   real(8) :: X_ao(DivisionNumber) = 0.010998655 !出口絶対湿度 [kg/kgDA]
   real(8) :: X_s(DivisionNumber) = 0.007422933 !シリカゲル表面絶対湿度 [kg/kgDA]
   real(8) :: Xs_assumed(DivisionNumber)
   real(8) :: T_r(DivisionNumber) = 25.0 !ローター温度 [℃]
   real(8) :: T_ao(DivisionNumber) = 25.0 !出口温度 [℃]
   real(8) :: P_old(DivisionNumber) = 0.05 !
   real(8) :: P_current(DivisionNumber), P_step_start(DivisionNumber)
   real(8) :: P0 = 1.0 
   real(8) :: RHs(DivisionNumber) = 37.72545 !シリカゲル表面相対湿度 [%]
   real(8) :: es(DivisionNumber) 
   real(8) :: Tai
   real(8) :: time_n !n分割した際の1ステップの時間 [s]
   real(8) :: error = 1.0e-8
   real(8) :: polanyi(DivisionNumber)
   real(8) :: qa, e(DivisionNumber), dRHs(DivisionNumber), dPolanyi(DivisionNumber), dP(DivisionNumber),de(DivisionNumber), dP_content(DivisionNumber), dXao(DivisionNumber), P_content(DivisionNumber)
   real(8) :: Tr_estimate(DivisionNumber),Tr_cal(DivisionNumber), Tao_cal(DivisionNumber), Xao_cal(DivisionNumber), Xs_new(DivisionNumber)
   ...
   do i = 1, 1800
      ...
      remainder = mod(i - 1, 360)
      !余りが0から179まではqa = qa_process(処理空気)、 180から359は qa = qa_regenerate(再生空気)
      if (remainder <= 179) then
      ...
      Tr_estimate = T_r !仮のTrを仮定、一ステップ前の確定温度を使う
      iter = 0 !ステップが進む時に無限ループ対策に使用しているカウンターをリセットする。
      P_step_start = P_old
